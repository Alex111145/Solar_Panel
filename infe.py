...
SOGLIA_DETECTION = 0.25  # Se lo score è sotto 0.25, viene scartato
NMS_THRESH = 0.40           
...
def main():
    ...
    print(f"\n📂 Analisi di {len(all_files)} patch...")
    try:
    ...
    for i, path in enumerate(files_to_process):
        ...
        # --- FILTRO SOGLIA MINIMA (ESPLICITO) ---
        # Teniamo solo le istanze che superano la SOGLIA_DETECTION
        keep_score = instances.scores > SOGLIA_DETECTION
        instances = instances[keep_score]
        ...
        if len(instances) > 0:
            ...
            # Se nessuna istanza supera la soglia, non colora nulla e logga
            print(f"📸 [{i+1}/{len(files_to_process)}] {fname} | Sotto soglia o nessun pannello.")
    ...
    print("\n" + "="*40)
    print(f"🎯 TOTALE PANNELLI UNICI (Sopra {SOGLIA_DETECTION}): {len(final_panels)}")
    print("="*40)
