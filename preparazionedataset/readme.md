...
1. Carica le immagini di `my_thesis_data/` su una piattaforma di annotazione (es. **Roboflow**).
2. Disegna i box (Bbox) o i poligoni attorno ai pannelli solari.
3. Esporta il dataset in formato **COCO JSON**.
4. Salva il file scaricato nel percorso:

```
datasets/solar_datasets/train/_annotations.coco.json
```
...
- **Calcolo GPS:**
  - Converte le posizioni dei pannelli dai singoli file alle coordinate globali del mosaico.
