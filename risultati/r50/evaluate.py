#!/usr/bin/env python
"""
evaluate.py - Valutazione MaskDINO Solar Panels (v5 - ResNet-50)
=================================================================
Coerente con train.py v5 (R50):
...
# ==============================================================================
# CONFIGURAZIONE — allineata con train.py v5
# ==============================================================================
...
def print_metrics_report(results):
    print("\n" + "=" * 65)
    print("📊  REPORT METRICHE DI VALUTAZIONE (v5 - R50)")
    print("=" * 65)
    ...
    if "segm" in results:
        ...
        print(f"  AP75       (mAP@75)    = {ap75:.2f}%  ← Target v5")
        ...
        if gap < 2:
            print(f"  🔴 Gap = {gap:.1f}% — quasi zero.")
            print(f"     Le mask sono sistematicamente troncate ai bordi.")
            print(f"     → Verifica che il training sia partito con resume=False.")
        elif gap < 8:
        ...
        if ap >= 52:
            ...
            print(f"  🟢 {ap:.1f}% → MOLTO BUONO")
        elif ap >= 40:
            print(f"  🟡 {ap:.1f}% → BUONO — margine di miglioramento")
        elif ap >= 35:
            print(f"  🟠 {ap:.1f}% → DISCRETO — margine di miglioramento")
        else:
            print(f"  🔴 {ap:.1f}% → Non soddisfacente. Verifica resume=False nel training.")
...
def main():
    parser = argparse.ArgumentParser(description="Valutazione MaskDINO Solar Panels v5 R50")
    ...
    print("\n🔎  VALUTAZIONE MASKDINO v5 — SOLAR PANELS (ResNet-50)")
    ...
    with open(report_path, "w") as f:
        f.write("METRICHE DI VALUTAZIONE MASKDINO v5 - ResNet-50\n")
        f.write("=" * 50 + "\n")
