#!/usr/bin/env python
"""
train.py - Training MaskDINO Solar Panels (v5 - ResNet-50)
===========================================================
[v5-R50] Backbone ResNet-50 invece di Swin-L
     MAX_ITER 30000
     STEPS (23000, 27000) — step decay
     WARMUP 500 iters
     LR 0.0001 ADAMW
     BACKBONE_MULTIPLIER 0.1 — R50 pre-trainato, LR backbone moderata
     CLASS_WEIGHT 4.0 — aumenta confidenze classificazione
     USE_CHECKPOINT False — R50 leggero, non serve gradient checkpointing
...
TRAIN_NAME = "solar_train_r50"
VALID_NAME = "solar_valid_r50"
OUTPUT_DIR = os.path.join(BASE_DIR, "output_solar_r50")
...
# ==========================================================================================
# 4. SETUP CONFIGURAZIONE — RESNET-50
# ==========================================================================================
...
def setup(args=None):
    ...
    # YAML per ResNet-50 (no cartella swin/)
    ...
    if not os.path.exists(YAML_PATH):
        print(f"❌ YAML R50 non trovato: {YAML_PATH}")
        sys.exit(1)

    cfg.merge_from_file(YAML_PATH)

    # Pesi pre-trainati ResNet-50
    R50_WEIGHTS = os.path.join(BASE_DIR, "weights", "maskdino_r50_pretrained.pth")
    if os.path.exists(R50_WEIGHTS):
        cfg.MODEL.WEIGHTS = R50_WEIGHTS
        print(f"  ✅ Pesi R50: {R50_WEIGHTS}")
    else:
        # Fallback: pesi COCO ufficiali da Detectron2 model zoo
        cfg.MODEL.WEIGHTS = "detectron2://ImageNetPretrained/MSRA/R-50.pkl"
        print(f"  ⚠️  Pesi R50 locali non trovati, uso model zoo: {cfg.MODEL.WEIGHTS}")
    ...
    # R50 molto più leggero di Swin-L → 2 img/GPU su 16GB
    IMAGES_PER_GPU    = 2
    ...
    cfg.SOLVER.BACKBONE_MULTIPLIER = 0.1   # R50: moltiplicatore più alto rispetto a Swin-L (0.05)
    cfg.SOLVER.WEIGHT_DECAY        = 0.05

    cfg.SOLVER.MAX_ITER     = 20000
    cfg.SOLVER.STEPS        = (13000, 17000)
    ...
    # R50 non richiede gradient checkpointing — rimosso USE_CHECKPOINT

    cfg.OUTPUT_DIR = OUTPUT_DIR
    os.makedirs(cfg.OUTPUT_DIR, exist_ok=True)

    print(f"\n{'='*65}")
    print(f"  MaskDINO Solar Panels — Training v5 (ResNet-50)")
    print(f"{'='*65}")
    print(f"  Backbone           : ResNet-50 (~25M param)")
    print(f"  GPUs               : {num_gpus}")
    print(f"  Batch size totale  : {GLOBAL_BATCH_SIZE} ({IMAGES_PER_GPU} img/GPU)")
...
def main(args=None):
    ...
    # resume=False — parte sempre da zero con pesi R50 pre-trainati
