...
(Il file contiene una riga JSON per ogni iterazione salvata dal CSVLogHook).
...
# Aggiornato per puntare alla cartella del nuovo training ResNet-50
CSV_DEFAULT = os.path.join(BASE_DIR, "output_solar_nuovor50", "metrics_all.csv")
...
# ==============================================================================
# 🔧 LETTURA DATI (JSON Lines)
# ==============================================================================

def carica_csv(path):
    """
    Legge il file generato dal CSVLogHook.
    Poiché il custom hook salva in formato JSON Lines (una riga = un dict JSON),
    usiamo json.loads per caricare i dati in modo dinamico e sicuro, gestendo 
    automaticamente il numero variabile di colonne introdotto dai Transformer.
    """
        ...
        for line in f:
            ...
            if line:
                try:
                    ...
                    continue  # Ignora righe parziali o corrotte

    if not righe:
        print("❌ File vuoto o formato non valido.")
        sys.exit(1)
    ...
    # Converti tutte le colonne in float (pandas ignorerà le stringhe nei KeyError)
    for c in df.columns:
        df[c] = pd.to_numeric(df[c], errors="coerce")

    print(f"✅ Dati caricati: {len(df)} iterazioni registrate")
...
def plot_main(df, output="training_curves.png"):
    ...
    print(f"\n   Loss trovate → total:{col_total} mask:{col_mask} cls:{col_cls} box:{col_box} lr:{col_lr}")
    print(f"   AP trovate   → AP:{col_ap}  AP50:{col_ap50}")

    fig = plt.figure(figsize=(16, 10), facecolor="white")
    # Aggiornato il titolo per la ResNet-50
    fig.suptitle("Curve di Apprendimento — MaskDINO (ResNet-50)", 
    ...
    if col_total:
        ...
        ax5.plot(itr[cutoff:], smooth(vals[cutoff:]), color=C_TOTAL, lw=2.0, label="Total Loss")
...
def plot_loss_detail(df, output="loss_detail.png"):
    ...
    fig.suptitle("Analisi delle Loss — MaskDINO (ResNet-50)",
