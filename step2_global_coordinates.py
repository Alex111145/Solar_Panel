...
def get_base_dir():
    ...
    # 2. Lista dei percorsi assoluti probabili (Mac, Lambda, Windows)
    possible_paths = [
...
def convert_to_global():
    if not os.path.exists(INPUT_JSON):
        print(f"❌ ERRORE: Non trovo il file {INPUT_JSON}.")
        print(f"🔍 Cercato in: {os.path.abspath(INPUT_JSON)}")
        return
    ...
    print(f"💾 File generato: {os.path.abspath(OUTPUT_JSON)}")
    print("-" * 50)
